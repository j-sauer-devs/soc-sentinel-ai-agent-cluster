...
Topology:
  commander ──fan-out──► triage ──────────┐
                  └────► threat_hunter ───┼──► oversight ──► briefing
                  └────► forensics ───────┘
                              ▲
                              │  (confidence < 70 AND iteration < 3)
                              └──────────────────────────────────────┘

The commander fans out to triage, threat_hunter, and forensics in parallel
via Send. LangGraph waits for all three branches before moving to oversight.
...
# ---------------------------------------------------------------------------
# Commander fan-out: dispatch all three specialists in parallel via Send
# ---------------------------------------------------------------------------

def commander_fanout(state: SOCState):
    """Return Send objects to dispatch triage, threat_hunter, forensics in parallel."""
    return [
        Send("triage", state),
...
def build_graph() -> StateGraph:
    ...
    # Commander fans out to all three specialists in parallel
    graph.add_conditional_edges(
        "commander",
        commander_fanout,
        ["triage", "threat_hunter", "forensics"],
    )

    # All three specialists converge at oversight
